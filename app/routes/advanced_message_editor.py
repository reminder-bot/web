...
def update_message(guild_id: int, reminder_uid: str):
    ...
    if field('embedded') is not None:
        ...
        if color.failed:
            ...
            author_url = icon if (icon := field('embed_author_url')).startswith('https://') else None
            ...
            reminder.embed_author = field('embed_author')
            reminder.embed_author_url = author_url
        ...
        db.session.query(EmbedField).filter(EmbedField.reminder_id == reminder.id).delete(synchronize_session='fetch')
